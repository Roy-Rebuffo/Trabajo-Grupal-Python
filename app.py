...
import inspect
from Modules.modules_register import modules
...
class Api:
    ...
    def register_module(self, name, module_instance):
        # Agrega todos los métodos públicos del módulo al API
        for attr_name in dir(module_instance):
            if not attr_name.startswith("_"):
                method = getattr(module_instance, attr_name)
                if callable(method):
                    # Crea un método dinámico en self con nombre: nombre_modulo_nombre_funcion
...
def main():
    ...
    api.window = window  # Assign the window to the API instance

    webview.start(debug=True) # Set debug to False for final build
